...
def export_onnx(
    ...
    if 1:
        ...
        output_path = os.path.join(output_folder, f"lighterglue_{top_k}_L{ligherglue_n_layers}.onnx")
        torch.onnx.export(
            matcher,
            (kpts, kpts, desc, desc),
            output_path,
...
if __name__ == "__main__":
    export_onnx(
        ...
        output_folder="onnx",
        input_shape=(1, 3, 600, 800),  # 输入形状, (1, C, H, W)
        ligherglue_n_layers=3,
        dynamic=False,  # 是否动态输入
        dense=False,    # 是否密集匹配
        top_k=512,     # 最大关键点数, 2048, 4096
    )
