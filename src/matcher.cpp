...
Matcher::Matcher(const std::string& model_path, const std::string& name, bool force_cpu, float threshold, cv::Size input_wh)
    : OrtRun(model_path, name, force_cpu), threshold_(threshold), 
      input_wh_(input_wh), output_matches_(nullptr), output_scores_(nullptr) {
}

bool Matcher::run(const std::vector<cv::Point2f>& keypoints1,
    ...
    if (input_tensors.empty()) {
        std::cerr << "Failed to preprocess matching data" << std::endl;
        return false;
    }
    ...
    if (!OrtRun::run(std::move(input_tensors), output_tensors)) {
        std::cerr << "Failed to run matching inference" << std::endl;
        return false;
    }
...
bool Matcher::preprocess(std::vector<Ort::Value>& /*inputs*/) {
    // 对于匹配，预处理已经在run函数中完成
    return true;
}

bool Matcher::postprocess(std::vector<Ort::Value>& /*outputs*/) {
    // 对于匹配，后处理已经在run函数中完成
    return true;
}

std::vector<Ort::Value> Matcher::preprocessMatchingData(
    ...
    try {
        ...
        auto norm_kpts1 = normalizeKeypoints(keypoints1, input_wh_.height, input_wh_.width);
        auto norm_kpts2 = normalizeKeypoints(keypoints2, input_wh_.height, input_wh_.width);
        ...
        auto kpts1_tensor = createTensor(kpts1_data,  kpts1_shape);
        auto kpts2_tensor = createTensor(kpts2_data,  kpts2_shape);
...
bool Matcher::postprocessMatchingResults(const std::vector<Ort::Value>& data,
                                        std::vector<cv::DMatch>& matches,
                                        std::vector<float>& scores) {
    if (data.size() < 2) {
    ...
    try {
        ...
        // 解析匹配结果
        float threshold = threshold_;  // 与Python代码draw_matches相同的阈值（不过滤）
...
std::vector<cv::Point2f> Matcher::normalizeKeypoints(const std::vector<cv::Point2f>& keypoints,
